...
// (utility cost prediction)

export default function GapAiEnergyConsumptionForecast() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-ai-energy-consumption-forecast', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 960, margin: '0 auto' }}>
      <h1 style={{ fontSize: 28, marginBottom: 6 }}>Ai Energy Consumption Forecast</h1>
      <p style={{ color: '#555', marginBottom: 20 }}>(utility cost prediction)</p>
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: 20, marginBottom: 20 }}>
        ...
        <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
          ...
          >
            {loading ? 'Running...' : 'Run Ai Energy Consumption Forecast'}
          </button>
          <button
