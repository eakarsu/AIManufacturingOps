...
// Route: /api/gap-ai-workforce-skill-gap
// Feature: Ai Workforce Skill Gap
// Description: (training needs from defect data)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Workforce Skill Gap. (training needs from defect data)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Workforce Skill Gap\nDescription: (training needs from defect data)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-workforce-skill-gap', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-workforce-skill-gap', title: 'Ai Workforce Skill Gap', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-workforce-skill-gap', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-workforce-skill-gap']);
      return res.json({ slug: 'ai-workforce-skill-gap', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-workforce-skill-gap', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-workforce-skill-gap', items: [], error: (e && e.message) || 'Server error' });
  }
});
