// Auto-generated feature route: production-planner-agent
// Domain: agentic production planner — Capability: balancing throughput, energy, and maintenance windows
// Mounted under: /api/ai/production-planner-agent
...
// POST /api/ai/production-planner-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a agentic production planner. Your task is balancing throughput, energy, and maintenance windows. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/ai/production-planner-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/ai/production-planner-agent/:id — fetch one
...
// DELETE /api/ai/production-planner-agent/:id — discard a run
