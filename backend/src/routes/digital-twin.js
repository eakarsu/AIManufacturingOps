// Auto-generated feature route: digital-twin
// Domain: digital twin scheduler — Capability: running what-if scheduling on a digital twin sandbox
// Mounted under: /api/ai/digital-twin
...
// POST /api/ai/digital-twin/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a digital twin scheduler. Your task is running what-if scheduling on a digital twin sandbox. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/ai/digital-twin/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/ai/digital-twin/:id — fetch one
...
// DELETE /api/ai/digital-twin/:id — discard a run
