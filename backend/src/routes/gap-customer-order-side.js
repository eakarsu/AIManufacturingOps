...
// Route: /api/gap-customer-order-side
// Feature: Customer Order Side
// Description: Customer/order-side portal
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Customer Order Side. Customer/order-side portal' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Customer Order Side\nDescription: Customer/order-side portal\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['customer-order-side', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'customer-order-side', title: 'Customer Order Side', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'customer-order-side', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['customer-order-side']);
      return res.json({ slug: 'customer-order-side', items: r.rows || [] });
    }
    return res.json({ slug: 'customer-order-side', items: [] });
  } catch (e) {
    return res.json({ slug: 'customer-order-side', items: [], error: (e && e.message) || 'Server error' });
  }
});
