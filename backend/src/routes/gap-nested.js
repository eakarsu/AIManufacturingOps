...
// Route: /api/gap-nested
// Feature: Nested
// Description: Nested routes structure (everything is monolithic in index.js)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Nested. Nested routes structure (everything is monolithic in index.js)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'nested', title: 'Nested', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'nested', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['nested']);
      return res.json({ slug: 'nested', items: r.rows || [] });
    }
    return res.json({ slug: 'nested', items: [] });
  } catch (e) {
    return res.json({ slug: 'nested', items: [], error: (e && e.message) || 'Server error' });
  }
});
