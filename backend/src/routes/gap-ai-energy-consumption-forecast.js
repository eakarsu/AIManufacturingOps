...
// Route: /api/gap-ai-energy-consumption-forecast
// Feature: Ai Energy Consumption Forecast
// Description: (utility cost prediction)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Energy Consumption Forecast. (utility cost prediction)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Energy Consumption Forecast\nDescription: (utility cost prediction)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-energy-consumption-forecast', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-energy-consumption-forecast', title: 'Ai Energy Consumption Forecast', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-energy-consumption-forecast', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-energy-consumption-forecast']);
      return res.json({ slug: 'ai-energy-consumption-forecast', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-energy-consumption-forecast', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-energy-consumption-forecast', items: [], error: (e && e.message) || 'Server error' });
  }
});
