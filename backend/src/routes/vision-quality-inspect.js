// Auto-generated feature route: vision-quality-inspect
// Domain: manufacturing vision QA inspector — Capability: classifying defects from a camera frame
// Mounted under: /api/ai/vision-quality-inspect
...
// POST /api/ai/vision-quality-inspect/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a manufacturing vision QA inspector. Your task is classifying defects from a camera frame. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/ai/vision-quality-inspect/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/ai/vision-quality-inspect/:id — fetch one
...
// DELETE /api/ai/vision-quality-inspect/:id — discard a run
