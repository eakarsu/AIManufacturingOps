// Auto-generated feature route: oee-anomaly-stream
// Domain: OEE anomaly detector — Capability: streaming OEE anomaly detection and ticket creation
// Mounted under: /api/ai/oee-anomaly-stream
...
// POST /api/ai/oee-anomaly-stream/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a OEE anomaly detector. Your task is streaming OEE anomaly detection and ticket creation. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/ai/oee-anomaly-stream/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/ai/oee-anomaly-stream/:id — fetch one
...
// DELETE /api/ai/oee-anomaly-stream/:id — discard a run
