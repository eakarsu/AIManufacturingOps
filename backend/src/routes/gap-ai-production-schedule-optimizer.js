...
// Route: /api/gap-ai-production-schedule-optimizer
// Feature: Ai Production Schedule Optimizer
// Description: (multi-line scheduling)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Production Schedule Optimizer. (multi-line scheduling)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Production Schedule Optimizer\nDescription: (multi-line scheduling)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-production-schedule-optimizer', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-production-schedule-optimizer', title: 'Ai Production Schedule Optimizer', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-production-schedule-optimizer', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-production-schedule-optimizer']);
      return res.json({ slug: 'ai-production-schedule-optimizer', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-production-schedule-optimizer', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-production-schedule-optimizer', items: [], error: (e && e.message) || 'Server error' });
  }
});
